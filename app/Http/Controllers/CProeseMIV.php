...
class CProeseMIV extends Controller
{
    ...
    public function cari_logdb_rcn(Request $request)
    {
        $vuserid   = $request->input('vuserid');
        $vtglawal  = $request->input('vtglawal');
        $vtglakhir = $request->input('vtglakhir');
        $vkdbank   = $request->input('vkdbank');
        ...
        $MyData = Http::withToken($request->session()->get('_datalogin.data.token'))
            ->post(config('myconfig.variable.SVR_URL_API') . 'getlogdbfilercn', [
                'vuserid'   => $vuserid,
                'vtglawal'  => $vtglawal,
                'vtglakhir' => $vtglakhir,
                'vkdbank'   => $vkdbank
